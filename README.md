# IsoSeqPse (version 0.2)
Time-stamp: <2018-02-11 Yunhao Wang, Email: yunhaowang@126.com>


## Introduction

Full-length isoform sequencing (Iso-Seq) technology originally developed by Pacific Biosciences (PacBio) has been widely applied to transcriptome study. IsoSeqPse is a bioinformatics tool to construct full-length transcriptome (including the detection of known gene isoforms and the prediction of novel gene isoforms) and identify transcribed pseudogenes using PacBio Iso-Seq data. In addition, Second Generation Sequencing (SGS, e.g., Illumina platform) short reads (RNA-Seq data) can also be used to improve the construction of full-length transcriptome.
...
1. Download the package (e.g., `IsoSeqPse-0.2.tar.gz`) to a directory (e.g., `/home/`)

2. Unpack it using the command `tar -zxvf /home/IsoSeqPse-0.2.tar.gz`

3. Now, you can run IsoSeqPse by the executable file `/home/IsoSeqPse-0.2/bin/isoseqpse`. Optional, you can add IsoSeqPse into your PATH so that you can run IsoSeqPse without having to specify the entire path. For example, you can add one line `export PATH=/home/IsoSeqPse-0.2/bin:$PATH` to your `~/.bashrc`
...
### 6. Pseudogene list file (TXT format)

The website `http://pseudogene.org/index.html` describes some resources about pseudogenes (including human and mouse).

The first column (split by 'tab') of TXT file should be pseudogene ID (gene ID and/or transcript ID). Multiple txt files can be as inputs split by "space".

## Output

### 1. Constructed pseudogenes (modified GPD format)
...
(20) Number of supporting Iso-Seq full-length long reads with unique alignment

(21) Number of supporting Iso-Seq full-length long reads with multiple alignment

(22) Number of supporting Iso-Seq long reads (both full-length and non-full-length) with unique alignment

(23) Number of supporting Iso-Seq long reads (both full-length and non-full-length) with multiple alignment

### 2. Constructed pseudogenes (GTF format)
...
20th column of GPD file

(12) fl_mult

21th column of GPD file
...
`./bin/isoseqpse -g ./example/input/SIRV_reference_genome.fasta -a ./example/input/SIRV_gene_annotation.gtf -l ./example/input/SIRV_ROI_sense.uniq ./example/input/SIRV_ROI_sense.mult ./example/input/SIRV_ROI_unknown.uniq ./example/input/SIRV_ROI_unknown.mult -c ./example/input/SIRV_ROI.primer_info.csv -s ./example/input/SIRV_SR.sam --pse_gene ./example/input/SIRV_pseudogene_list.txt --tempdir ./example/temp_with_SR/ --output_gpd ./example/SIRV_test_with_SR.gpd --output_gtf ./example/SIRV_test_with_SR.gtf`

### 2. No SGS short read alignment data

`./bin/isoseqpse -g ./example/input/SIRV_reference_genome.fasta -a ./example/input/SIRV_gene_annotation.gtf -l ./example/input/SIRV_ROI_sense.uniq ./example/input/SIRV_ROI_sense.mult ./example/input/SIRV_ROI_unknown.uniq ./example/input/SIRV_ROI_unknown.mult -c ./example/input/SIRV_ROI.primer_info.csv --pse_gene ./example/input/SIRV_pseudogene_list.txt --tempdir ./example/temp_without_SR/ --output_gpd ./example/SIRV_test_without_SR.gpd --output_gtf ./example/SIRV_test_without_SR.gtf`
